// v1.10 step2 verify: knowledge tree prerequisite arrows (count + cross-tag count).
...
const dir = path.join(os.tmpdir(), 'chrome-cdp-v110c-' + Date.now());
const OUT = process.argv[2] || (path.join(os.tmpdir(), 'cdp_v110c_evidence.json'));
...
async function main(){
  ...
  const expr = `(async()=>{
    ...
    tabs[ti].click(); await new Promise(r=>setTimeout(r,2200));
    const page=document.getElementById('page-tree');
    const out={};
    let svg=page.querySelector('.ktree');
    ...
    // expand all tags (all start folded, so clicking each one expands it)
    tagGs.forEach(function(g){g.dispatchEvent(new MouseEvent('click',{bubbles:true}));});
    await new Promise(r=>setTimeout(r,600));
    svg=page.querySelector('.ktree');
    out.arrows=svg?svg.getAttribute('data-arrows'):null;
    out.crossTag=svg?svg.getAttribute('data-cross'):null;
    out.arrowEls=svg?svg.querySelectorAll('.prereq-arrow').length:0;
    out.crossEls=svg?svg.querySelectorAll('.prereq-arrow[data-cross="1"]').length:0;
